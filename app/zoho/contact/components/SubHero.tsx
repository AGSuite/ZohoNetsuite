...
export default function SubHero({ title, subtitle, breadcrumb, gradientText }: SubHeroProps) {
    return (
        <section className="relative h-[50vh] min-h-[400px] overflow-hidden bg-gradient-to-br from-[#1a0000] via-[#2a0505] to-[#0a0a0a] flex items-center pt-20">
            {/* Grid lines */}
            <div
            ...
            {/* Glow blobs */}
            <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-red-600/15 rounded-full blur-[120px] -translate-x-1/3 -translate-y-1/4 pointer-events-none" />
            <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-orange-600/10 rounded-full blur-[100px] translate-x-1/3 -translate-y-1/4 pointer-events-none" />

            {/* Floating particles */}
            {PARTICLES.map((p, i) => (
                <motion.div
                    key={i}
                    className="absolute rounded-full bg-red-400/40 pointer-events-none"
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-6 w-full text-center lg:text-left">
                ...
                >
                    ...
                    <Link href="/zoho/contact" className="text-red-300 hover:text-white transition-colors">Contact</Link>
                    <ChevronRight className="w-3.5 h-3.5 text-white/30" />
